...
class EmailAuthViewModel: EmailAuthRes {
    ...
    fileprivate let errorResponseSubject = PublishSubject<TripService.Err>()
    ...
    static func create(purpose: Purpose,
        ...
        switch purpose {
        case .signIn:   return SignViewModel(input: input)
    ...
    fileprivate class ValidationService {
        ...
        static func checkEmail(_ email: String, withResponsedError err: TripService.Err) -> String {
            guard !email.isEmpty else {
                return TripService.Err.empty.description
            }
        ...
        static func checkPassword(_ password: String, withResponsedError err: TripService.Err) -> String {
        ...
        static func filterResponsedError(_ err: TripService.Err) -> Bool {
...
class SignUpViewModel: EmailAuthViewModel {
    ...
    override var pageTitle: String { return "Sign Up" }
    ...
    fileprivate override init(input: (
        ...
        completionDrv = input.actionTap.withLatestFrom(emailAndPw)
            .flatMap { (pair) in
                return TripService.createUser(withEmail: pair.email, password: pair.password)
                    .map({ (response) -> User? in
                        switch response {
                        case .success(let resp):
                            return resp
                        case .fail( _):
                            self.errorResponseSubject.onNext(.unauthenticated)
                            return nil
                        }
                    })
...
class SignViewModel: EmailAuthViewModel {
    ...
    fileprivate override init(input: (
        ...
        completionDrv = input.actionTap.withLatestFrom(emailAndPw)
            .flatMap { (pair) in
                return TripService.signIn(withEmail: pair.email, password: pair.password)
                    .map({ (response) -> User? in
                        switch response {
                        case .success(let resp):
                            return resp
